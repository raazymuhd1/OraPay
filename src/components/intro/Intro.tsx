...
const Intro = () => {
  return (
    <section className="min-h-[600px] flex flex-col items-center justify-center w-[vw] gap-[10px] p-[40px]">
        ...
        <div className="grid lg:grid-cols-[repeat(3,minmax(200px,1fr))] grid-cols-[repeat(auto-fit,minmax(150px,1fr))] w-full lg:w-[80%] gap-[30px] justify-center mt-[60px]">
            { intros.map(intro => (
                <div key={intro.id} 
                    className="flex flex-col gap-[10px] border-[1px] border-[var(--dark-border)] min-h-[250px] rounded-[20px] p-[20px] hover:translate-y-[-10px] transition-all duration-500 cursor-pointer intro-card glass-card">
