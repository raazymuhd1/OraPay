...
const CustomButton = forwardRef(({ children, onClick, disabled, style, mouseEvent, mouseLeave}: IProps, ref: Ref<HTMLButtonElement>) => {
  ...
  return (
    <button 
        ...
        className={`flex items-center justify-center gap-[10px] whitespace-nowrap text-white font-semibold ${disabled ? "cursor-not-allowed opacity-[.8]" : "cursor-pointer"} rounded-[10px] py-[6px] px-[10px] ${style} `}>
        { children }
    </button>
  )
})
