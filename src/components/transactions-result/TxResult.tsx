...
const TxResult = ({ title, msg, showTxResult, setShowTxResult, closeTxResult }: IProps) => {
  ...
  return (
    // <div className={`w-full h-screen fixed top-0 glass-modal`}>
        <article className={`${showTxResult ? "w-full opacity-[1] translate-y-[90px] absolute top-[60px] left-0 h-[60%] grid grid-cols=1 grid-rows-[repeat(2,minmax(50px,150px))]" : "opacity-[0] translate-y-[100px]"} gap-[10px] p-[20px]  rounded-[10px] transition-all items-center duration-500 glass-card z-[9]`}>
